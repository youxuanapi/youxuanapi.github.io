    ...
    return lines.map((line, index) => {
      ...
      if (isHeading) {
        return (
          <h3
            key={index}
            className="text-lg font-bold text-[#5c6bc0] mb-4 mt-8 border-l-4 border-[#5c6bc0] pl-3 bg-indigo-50/50 py-1"
          >
            {line}
          </h3>
        );
    ...
    <div className={cn('flex flex-col h-full bg-gray-50', className)}>
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-6xl mx-auto space-y-6">
              ...
              <div className="inline-flex bg-gray-100 rounded-full p-1.5 shadow-inner">
                ...
                <button
                  ...
                  className={cn(
                    'px-8 py-2.5 rounded-full font-semibold transition-all duration-300 flex items-center gap-2',
                    activeMode === 'humanize'
                      ? 'bg-gradient-to-r from-purple-600 to-purple-700 text-white shadow-lg shadow-purple-200 scale-105'
                      : 'text-gray-600 hover:text-gray-900 hover:bg-gray-200'
                  )}
                >
                  🛡️ 深度拟人润色
              ...
              <div>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <label htmlFor="antiAiMode" className="text-sm font-medium text-gray-700">
                    ✨ 开启高阶真实感引擎 (Human-Touch Mode)
                  </label>
                </div>
                <p className="text-xs text-gray-500 mb-4 ml-7">
                  开启后，系统将深度模拟真实人类的叙事逻辑、情绪波动与语言习惯，消除刻板的机器味，提升文本亲和力。
                </p>
                ...
                {activeMode === 'create' && showSkeletonPanel && (
              ...
              <div className="flex gap-3">
                {!currentTask && !isRunning && (
                  <button
                    ...
                    className={cn(
                      'px-6 py-2 text-white rounded-lg font-semibold shadow-lg transition-all duration-300 hover:shadow-xl hover:scale-[1.02]',
                      activeMode === 'create' 
                        ? 'bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800' 
                        : 'bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800'
                    )}
                  >
                    {activeMode === 'create' ? '🚀 开始生成原创爆文' : '🛡️ 开始注入真实情感'}
                  </button>
                )}
                ...
                {activeMode === 'create' && currentTask?.status === 'outline_pending' && !isRunning && (
                  <div className="flex gap-3">
                    <button
                      ...
                      className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-semibold shadow-lg transition-all duration-300 hover:shadow-xl"
                    >
                      确认大纲，开始生成
                    </button>
                    <button
                      onClick={reset}
                      className="px-6 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 font-semibold shadow-lg transition-all duration-300 hover:shadow-xl"
                    >
                      重新生成大纲
                    </button>
                  </div>
                ...
                {isRunning && (
                  <button
                    onClick={cancelTask}
                    className="px-6 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 font-semibold shadow-lg transition-all duration-300 hover:shadow-xl"
                  >
                    取消任务
                  </button>
                )}
                ...
                {currentTask?.finalDraft && (
                  <button
                    onClick={reset}
                    className="px-6 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 font-semibold shadow-lg transition-all duration-300 hover:shadow-xl"
                  >
                    新建任务
                  </button>
                )}
                ...
                {activeMode === 'create' && isRunning && (
                  <div className="space-y-4">
                    <div className="bg-slate-900 rounded-lg p-4 font-mono text-sm">
                      <div className="text-green-400 mb-2 font-bold">🔧 系统状态</div>
                  ...
                  <div className="border border-gray-200 rounded-lg overflow-hidden bg-gray-50">
                    ...
                    <div className="p-4">
                      {currentTask.outline.sections && Array.isArray(currentTask.outline.sections) && currentTask.outline.sections.length > 0 ? (
                        <ul className="list-disc pl-5 space-y-2">
                          {currentTask.outline.sections.map((section: any, index: number) => (
                            <li key={index} className="text-gray-700">
                              <p className="font-medium text-gray-900">{section.corePosition}</p>
                        ...
                        <p className="text-gray-400 text-sm">大纲内容为空，请尝试重新生成</p>
                      )}
                    </div>
                  </div>
                )}
                      ...
                      {currentTask.finalDraft && (
                        <div className="flex gap-2">
                          <button
                            ...
                            className="px-3 py-1 text-xs rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors font-medium shadow"
                          >
                            🪄 {isFormatted ? '取消排版' : '一键排版'}
                          </button>
                          <button
                            onClick={handleCopy}
                            className={cn(
                              'px-3 py-1 text-xs rounded-lg transition-all duration-200 font-medium shadow',
                              copied 
          ...
          <div className="text-center text-[13px] text-gray-400 mt-8 pb-4 border-t border-gray-100 pt-4 leading-relaxed">
            💡 平台合规提示：本系统提供的辅助创作与文本优化服务仅供灵感参考与效率提升。请严格遵守各内容分发平台的发布规范，严禁将本工具用于学术造假、制造虚假新闻、违规引流等不当用途。
          </div>
        </div>
      </div>
    </div>
